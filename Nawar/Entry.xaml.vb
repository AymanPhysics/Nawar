...
Public Class Entry
    ...
    Private Sub LoadWFH()
        ...
        If Not (Md.MyProjectType = ProjectType.NawarGroup OrElse Md.MyProjectType = ProjectType.X0 OrElse Md.MyProjectType = ProjectType.X27) Then
        ...
        Try
            ...
            ElseIf G.Columns(e.ColumnIndex).Name = GC.SubAccNo Then
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    If dt.Rows(0)("TableName") = "Suppliers" AndAlso Val(G.Rows(G.CurrentCell.RowIndex).Cells(GC.StoreInvoiceNo).Value) = 0 Then
            ...
            ElseIf G.Columns(e.ColumnIndex).Name = GC.ImportMessageId Then
                ...
                If Val(G.Rows(e.RowIndex).Cells(GC.ImportMessageId).Value) > 0 AndAlso bm.IF_Exists("select dbo.GetAccName(AccNo) from ImportMessages  where OrderTypeId='" & G.Rows(G.CurrentCell.RowIndex).Cells(GC.PurchaseAccNo).Value & "' and Id=" & Val(G.Rows(e.RowIndex).Cells(GC.ImportMessageId).Value) & " and (IsDelivered=0 or DeliveredDate<'" & bm.ToStrDate(DayDate.SelectedDate) & "')") Then
    ...
    Sub FillControls()
        If lop OrElse EntryTypeId.SelectedIndex < 1 Then Return
        lop = True
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        For i As Integer = 0 To G.Rows.Count - 1
            ...
            dt = bm.ExecuteAdapter("select C.LinkFile from Chart C where C.Id='" & G.Rows(i).Cells(GC.MainAccNo).Value & "'")
            If dt.Rows.Count > 0 Then
                If G.Columns(GC.CostTypeId).Visible AndAlso Val(dt.Rows(0)(0)) = 9 AndAlso Val(G.Rows(i).Cells(GC.CostTypeId).Value) = 0 Then
                    If bm.ShowDeleteMSG("هل تريد تحديد نوع التكلفة بالسطر " & (i + 1).ToString) Then
                        G.Focus()
            ...
            If Val(G.Rows(i).Cells(GC.CostTypeId).Value) = 2 AndAlso Val(G.Rows(i).Cells(GC.PurchaseAccNo).Value) = 0 Then
                bm.ShowMSG("برجاء تحديد الطلبية")
                G.Focus()
                G.CurrentCell = G.Rows(i).Cells(GC.PurchaseAccNo)
                Return
            End If


            If Val(G.Rows(i).Cells(GC.CostTypeId).Value) = 3 AndAlso Val(G.Rows(i).Cells(GC.ImportMessageId).Value) = 0 Then
                bm.ShowMSG("برجاء تحديد الرسالة")
                G.Focus()
                G.CurrentCell = G.Rows(i).Cells(GC.ImportMessageId)
                Return
            ...
            If Val(G.Rows(i).Cells(GC.CostTypeId).Value) = 4 AndAlso Val(G.Rows(i).Cells(GC.StoreInvoiceNo).Value) = 0 Then
