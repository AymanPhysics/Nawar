...
Public Class ImportMessages
    ...
    Structure GC
        ...
        Shared DocNo2 As String = "DocNo2"
    ...
    Private Sub LoadWFH()
        ...
        G.Columns.Add(GC.DocNo2, "رقم إذن صرف المورد")
        ...
        G.Columns(GC.DocNo2).ReadOnly = True
    ...
    Private Sub LoadWFH1()
        ...
        G1.Columns.Add(GC1.SalesInvoiceNo, "أمر الشراء")
        ...
        G1.Columns(GC1.SalesInvoiceNo).ReadOnly = True
    ...
    Private Sub LoadWFH2()
        ...
        G2.Columns.Add(GC2.SalesInvoiceNo, "أمر الشراء")
        ...
        G2.Columns(GC2.SalesInvoiceNo).ReadOnly = True
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If ShipperId.Visibility = Windows.Visibility.Visible AndAlso ShipperId.SelectedIndex < 1 Then
            bm.ShowMSG("برجاء تحديد " & lblShipperId.Content)
            ShipperId.Focus()
            Return
        End If
        If ShippingLineId.Visibility = Windows.Visibility.Visible AndAlso ShippingLineId.SelectedIndex < 1 Then
            bm.ShowMSG("برجاء تحديد " & lblShippingLineId.Content)
            ShippingLineId.Focus()
            Return
        End If
        If ShippingCompanyId.Visibility = Windows.Visibility.Visible AndAlso ShippingCompanyId.SelectedIndex < 1 Then
            bm.ShowMSG("برجاء تحديد " & lblShippingCompanyId.Content)
            ShippingCompanyId.Focus()
            Return
        End If
        If ContainerSizeId.Visibility = Windows.Visibility.Visible AndAlso ContainerSizeId.SelectedIndex < 1 Then
            bm.ShowMSG("برجاء تحديد " & lblContainerSizeId.Content)
            ContainerSizeId.Focus()
            Return
        End If

        If IsDelivered.IsChecked AndAlso DeliveredDate.SelectedDate < DayDate.SelectedDate Then
            bm.ShowMSG("برجاء تحديد تاريخ استلام صحيح")
            DeliveredDate.Focus()
            Return
        End If
        ...
        For i As Integer = 0 To G.Rows.Count - 1
            ...
            str &= " Insert " & TableNameDetails & "(" & MainId & "," & SubId & ",StoreId,InvoiceNo,SupplierId,SupplierName,DocNo,DocNo2,DeliveryDate,Perc"
            ...
            Try
                ...
                End If
                str &= "('" & OrderTypeId.Text & "','" & Val(txtID.Text) & "','" & Val(StoreId.Text) & "','" & G.Rows(i).Cells(GC.InvoiceNo).Value.ToString & "','" & G.Rows(i).Cells(GC.SupplierId).Value.ToString & "','" & G.Rows(i).Cells(GC.SupplierName).Value.ToString & "','" & G.Rows(i).Cells(GC.DocNo).Value.ToString & "','" & G.Rows(i).Cells(GC.DocNo2).Value.ToString & "','" & x & "','" & G.Rows(i).Cells(GC.Perc).Value & "'"
        ...
        bm.ExecuteNonQuery(str)
        str = " Insert " & TableNameDetailsSub & "(" & MainId & "," & SubId & ",StoreId,SalesInvoiceNo,InvoiceNo,ItemId,ItemName,Qty,Qty2,Qty3,ReturnQty,NetQty,QtyRecieved,Price,Value,SerialNo) values "
        For i As Integer = 0 To G1.Rows.Count - 1
            Try
                str &= "('" & OrderTypeId.Text & "','" & Val(txtID.Text) & "','" & Val(StoreId.Text) & "','" & G1.Rows(i).Cells(GC1.SalesInvoiceNo).Value.ToString & "','" & G1.Rows(i).Cells(GC1.InvoiceNo).Value.ToString & "','" & G1.Rows(i).Cells(GC1.ItemId).Value.ToString & "','" & G1.Rows(i).Cells(GC1.ItemName).Value.ToString & "','" & G1.Rows(i).Cells(GC1.Qty).Value.ToString & "','" & G1.Rows(i).Cells(GC1.Qty2).Value.ToString & "','" & G1.Rows(i).Cells(GC1.Qty3).Value.ToString & "','" & G1.Rows(i).Cells(GC1.ReturnQty).Value.ToString & "','" & G1.Rows(i).Cells(GC1.NetQty).Value.ToString & "','" & Val(G1.Rows(i).Cells(GC1.QtyRecieved).Value) & "','" & G1.Rows(i).Cells(GC1.Price).Value.ToString & "','" & G1.Rows(i).Cells(GC1.Value).Value.ToString & "','" & Val(G1.Rows(i).Cells(GC1.SerialNo).Value) & "'),"
            Catch ex As Exception
            End Try
        Next
    ...
    Private Sub FillControls()
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            ...
            G.Rows(i).Cells(GC.DocNo2).Value = dt.Rows(i)("DocNo2").ToString
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            G2.Rows.Add()
            G2.Rows(i).Cells(GC2.SalesInvoiceNo).Value = dt.Rows(i)("SalesInvoiceNo").ToString
    ...
    Private Sub IsDelivered_Checked(sender As Object, e As RoutedEventArgs) Handles IsDelivered.Checked
        ...
        'btnCalc3.Visibility = Windows.Visibility.Visible
    End Sub
    ...
    Private Sub IsDelivered_UnChecked(sender As Object, e As RoutedEventArgs) Handles IsDelivered.Unchecked
        ...
        'btnCalc3.Visibility = Windows.Visibility.Hidden
    End Sub
    ...
    Private Sub GridCalcRow(sender As Object, e As Forms.DataGridViewCellEventArgs)
        If G.Rows(e.RowIndex).Cells(e.ColumnIndex).Value = Nothing Then
            ...
            G.Rows(e.RowIndex).Cells(GC.DocNo2).Value = Nothing
        ...
        If G.Columns(e.ColumnIndex).Name = GC.InvoiceNo Then
            ...
            dt = bm.ExecuteAdapter("select ToId,dbo.GetSupplierName(ToId)ToName,DocNo,DocNo2,dbo.ToStrDate(DeliveryDate)DeliveryDate,TotalAfterDiscount,dbo.GetCurrencyName(CurrencyId)CurrencyName,Perc from SalesMaster where StoreId=" & StoreId.Text & " and Flag=" & Sales.FlagState.الاستيراد & " and InvoiceNo=" & G.Rows(e.RowIndex).Cells(GC.InvoiceNo).Value)
            If dt.Rows.Count > 0 Then
                ...
                G.Rows(e.RowIndex).Cells(GC.DocNo2).Value = dt.Rows(0)("DocNo2").ToString
...
a:
            ...
            For i As Integer = G2.Rows.Count - 1 To 0 Step -1
                If Val(G2.Rows(i).Cells(GC2.InvoiceNo).Value) = 0 Then Continue For
                For x As Integer = 0 To G.Rows.Count - 1
                    If G2.Rows(i).Cells(GC2.InvoiceNo).Value <> G.Rows(x).Cells(GC.InvoiceNo).Value Then Continue For
                    GoTo b
                Next
                G2.Rows.RemoveAt(i)
b:
    ...
    Private Sub GridKeyDown(sender As Object, e As Forms.KeyEventArgs)
        ...
        Try
            ...
            If G.CurrentCell.ColumnIndex = G.Columns(GC.InvoiceNo).Index Then
                bm.ShowHelpGrid("الفواتير", G.CurrentRow.Cells(GC.InvoiceNo), G.CurrentRow.Cells(GC.InvoiceNo), e, "select cast(InvoiceNo as varchar(100)) Id,dbo.GetSupplierName(ToId) Name from SalesMaster where StoreId=" & StoreId.Text & " and Flag=" & Sales.FlagState.الاستيراد & " and OrderTypeId=" & OrderTypeId.Text & " and Temp=0", , "الفاتورة", "المورد")
    ...
    Private Sub GridSelectionChanged(sender As Object, e As EventArgs)
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            G1.Rows.Add()
            G1.Rows(i).Cells(GC1.SalesInvoiceNo).Value = dt.Rows(i)("SalesInvoiceNo").ToString
    ...
    Private Sub G1_CellDoubleClick(sender As Object, e As Forms.DataGridViewCellEventArgs)
        ...
        For i As Integer = 0 To G2.Rows.Count - 1
            If G2.Rows(i).Cells(GC2.SalesInvoiceNo).Value = G1.Rows(e.RowIndex).Cells(GC1.SalesInvoiceNo).Value AndAlso G2.Rows(i).Cells(GC2.InvoiceNo).Value = G1.Rows(e.RowIndex).Cells(GC1.InvoiceNo).Value AndAlso G2.Rows(i).Cells(GC2.ItemId).Value = G1.Rows(e.RowIndex).Cells(GC1.ItemId).Value AndAlso G2.Rows(i).Cells(GC2.Price).Value = G1.Rows(e.RowIndex).Cells(GC1.Price).Value Then
        ...
        Else
            ...
            G2.Rows(i).Cells(GC2.SalesInvoiceNo).Value = G1.Rows(e.RowIndex).Cells(GC1.SalesInvoiceNo).Value
    ...
    Private Sub G2_CellEndEdit(sender As Object, e As Forms.DataGridViewCellEventArgs)
        ...
        Try
            For i As Integer = 0 To G1.Rows.Count - 1
                If G1.Rows(i).Cells(GC1.SalesInvoiceNo).Value = G2.Rows(e.RowIndex).Cells(GC2.SalesInvoiceNo).Value AndAlso G1.Rows(i).Cells(GC1.InvoiceNo).Value = G2.Rows(e.RowIndex).Cells(GC2.InvoiceNo).Value AndAlso G1.Rows(i).Cells(GC1.ItemId).Value = G2.Rows(e.RowIndex).Cells(GC2.ItemId).Value AndAlso G1.Rows(i).Cells(GC1.Price).Value = G2.Rows(e.RowIndex).Cells(GC2.Price).Value Then

                    Dim count As Integer = Val(G1.Rows(i).Cells(GC1.NetQty).Value)
                    For i2 As Integer = 0 To G2.Rows.Count - 1
                        If G2.Rows(i2).Cells(GC2.SalesInvoiceNo).Value = G1.Rows(i).Cells(GC1.SalesInvoiceNo).Value AndAlso G2.Rows(i2).Cells(GC2.InvoiceNo).Value = G1.Rows(i).Cells(GC1.InvoiceNo).Value AndAlso G2.Rows(i2).Cells(GC2.ItemId).Value = G1.Rows(i).Cells(GC1.ItemId).Value AndAlso G2.Rows(i2).Cells(GC2.Price).Value = G1.Rows(i).Cells(GC1.Price).Value Then
            ...
            If Not IsExists Then
                For x As Integer = 0 To G1.Rows.Count - 1
                    If MyId = G1.Rows(x).Cells(GC1.ItemId).Value Then
                        ...
                        Dim i As Integer = G2.Rows.Add
                        G2.Rows(i).Cells(GC2.SalesInvoiceNo).Value = G1.Rows(x).Cells(GC1.SalesInvoiceNo).Value
    ...
    Sub CalcImportMessageCostSubData()
        If DeliveredDate.SelectedDate Is Nothing Then
            ...
            Return
        End If
        Cost_Administrative0.Text = Val(bm.ExecuteScalar("CalcImportMessageCos
tSubData", {"DeliveredDate"}, {bm.ToStrDate(DeliveredDate.SelectedDate)}))
        bm.ExecuteNonQuery("exec GenerateCurrencyExchangeByDateTemp null,'" & bm.ToStrDate(DeliveredDate.SelectedDate) & "'")
        Cost_OrderTypeId0.Text = Val(bm.ExecuteScalar("select dbo.Bal0(AccNo,Id,'" & bm.ToStrDate(DeliveredDate.SelectedDate) & "','" & bm.ToStrDate(DeliveredDate.SelectedDate) & "',0,0,0)*C.Exchange from OrderTypes T
