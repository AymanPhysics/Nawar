...
Public Class DeliveryOrder
    ...
    Private Sub InvoiceNo_KeyUp(sender As Object, e As KeyEventArgs) Handles InvoiceNo.KeyUp, DocNo.KeyUp
        Dim title As String = ""
        If Parent.GetType Is GetType(Page) Then
            title = CType(Parent, Page).Title
        ElseIf Parent.GetType Is GetType(MyWindow) Then
            title = CType(Parent, MyWindow).Title
        End If

        Select Case Flag
            Case 0
                If bm.ShowHelpMultiColumns(title, InvoiceNo, InvoiceNo, e, "select cast(InvoiceNo as nvarchar(100))'رقم المسلسل',cast(ToId as nvarchar(100))'كود العميل',dbo.GetCustomerName(ToId)'اسم العميل',dbo.ToStrDate(DayDate)'التاريخ',DocNo 'رقم أمر الصرف',SalesInvoiceNo 'رقم كشف الحساب' from DeliveryOrderMaster where StoreId=" & Val(StoreId.Text) & " and Flag=" & Flag.ToString) Then
                    'InvoiceNo.Text = bm.SelectedRow(0)
                    InvoiceNo_Leave(Nothing, Nothing)
                End If
            Case 1
                If bm.ShowHelpMultiColumns(title, InvoiceNo, InvoiceNo, e, "select cast(InvoiceNo as nvarchar(100))'رقم المسلسل',cast(ToId as nvarchar(100))'كود المورد',dbo.GetSupplierName(ToId)'اسم المورد',dbo.ToStrDate(DayDate)'التاريخ',DocNo 'رقم أمر الصرف',SalesInvoiceNo 'رقم كشف الحساب' from DeliveryOrderMaster where StoreId=" & Val(StoreId.Text) & " and Flag=" & Flag.ToString) Then
                    'InvoiceNo.Text = bm.SelectedRow(0)
                    InvoiceNo_Leave(Nothing, Nothing)
                End If
        End Select
    ...
    Sub ClearControls()
        Try
            ...
            txtFlag.Text = Flag
            If Md.MyProjectType <> ProjectType.X12 AndAlso Md.MyProjectType <> ProjectType.X2 AndAlso Md.MyProjectType <> ProjectType.X3 Then CashierId.Text = Md.UserName
    ...
    Private Sub DocNo_LostFocus(sender As Object, e As RoutedEventArgs) Handles DocNo.LostFocus
        ...
        Dim dt2 As DataTable = bm.ExecuteAdapter("GetSalesSerialDetails", {"Flag", "StoreId", "DocNo", "SalesTypeIdState"}, {Flag, Val(StoreId.Text), Val(DocNo.Text), IIf(Md.MyProjectType = ProjectType.NawarGroup, 0, 1)})
        G.Rows.Clear()
