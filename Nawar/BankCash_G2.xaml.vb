...
Public Class BankCash_G2
    ...
    Private Sub BasicForm_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        ...
        If Not Md.MyProjectType = ProjectType.NawarGroup Then
            ...
            btnIsSelected.Visibility = Windows.Visibility.Hidden
        End If
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        For i As Integer = 0 To G1.Rows.Count - 1
            ...
            If Val(G1.Rows(i).Cells(GC.CheckTypeId).Value) = 2 AndAlso bm.IF_Exists("select * from BankCash_G2 where Canceled=0 and CheckTypeId=2 and CheckNo='" & G1.Rows(i).Cells(GC.CheckNo).Value & "' and not(Flag=" & Flag & " and BankCash_G2TypeId=" & BankCash_G2TypeId.SelectedValue & " and InvoiceNo=" & Val(txtID.Text) & ")") Then
            ...
            If Md.MyProjectType = ProjectType.NawarGroup AndAlso G1.Columns(GC.CostTypeId).Visible AndAlso Val(G1.Rows(i).Cells(GC.LinkFile).Value) = 9 AndAlso Val(G1.Rows(i).Cells(GC.CostTypeId).Value) = 0 Then
            ...
            If Val(G1.Rows(i).Cells(GC.CostTypeId).Value) = 2 AndAlso Val(G1.Rows(i).Cells(GC.PurchaseAccNo).Value) = 0 Then
                bm.ShowMSG("برجاء تحديد الطلبية")
                G1.Focus()
                G1.CurrentCell = G1.Rows(i).Cells(GC.PurchaseAccNo)
                Return
            End If


            If Val(G1.Rows(i).Cells(GC.CostTypeId).Value) = 3 AndAlso Val(G1.Rows(i).Cells(GC.ImportMessageId).Value) = 0 Then
                bm.ShowMSG("برجاء تحديد الرسالة")
                G1.Focus()
                G1.CurrentCell = G1.Rows(i).Cells(GC.ImportMessageId)
                Return
            End If

            If Val(G1.Rows(i).Cells(GC.CostTypeId).Value) = 4 AndAlso Val(G1.Rows(i).Cells(GC.StoreInvoiceNo).Value) = 0 Then
                bm.ShowMSG("برجاء تحديد الفاتورة")
                G1.Focus()
                G1.CurrentCell = G1.Rows(i).Cells(GC.StoreInvoiceNo)
                Return
            End If
        Next
    ...
    Private Sub GridCalcRow(ByVal sender As Object, ByVal e As Forms.DataGridViewCellEventArgs)
        ...
        If lop Then Return
        Try
            If Md.MyProjectType = ProjectType.NawarGroup AndAlso e.RowIndex < G.Rows.Count - 1 AndAlso Val(G.Rows(e.RowIndex).Cells(GC.DocNo).Value) = 0 Then
            ...
            If G.Columns(e.ColumnIndex).Name = GC.MainValue OrElse G.Columns(e.ColumnIndex).Name = GC.Exchange Then
                ...
                G.Rows(e.RowIndex).Cells(GC.Value).Value = Val(G.Rows(e.RowIndex).Cells(GC.Exchange).Value) * Val(G.Rows(e.RowIndex).Cells(GC.MainValue).Value)
            ...
            ElseIf G.Columns(e.ColumnIndex).Name = GC.SubAccNo Then
                If Md.ShowBankCash_GAccNo_Not_LinkFile Then
                    ...
                    If dt.Rows.Count > 0 Then
                        ...
                        If Val(G.Rows(e.RowIndex).Cells(GC.LinkFile).Value) = 2 AndAlso Val(G.Rows(e.RowIndex).Cells(GC.StoreInvoiceNo).Value) = 0 Then
                            'bm.ShowMSG("برجاء إدخال رقم فاتورة المورد")
                        End If
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    Else
                        ...
                        If Val(G.Rows(e.RowIndex).Cells(GC.Exchange2).Value) = 0 Or Val(G.Rows(e.RowIndex).Cells(GC.MainValue2).Value) = 0 Then
                            ...
                            G.Rows(e.RowIndex).Cells(GC.MainValue2).Value = G.Rows(e.RowIndex).Cells(GC.Value).Value / G.Rows(e.RowIndex).Cells(GC.Exchange2).Value
                        End If
                    End If
                End If
            ...
            ElseIf G.Columns(e.ColumnIndex).Name = GC.ImportMessageId Then
                ...
                If Val(G.Rows(e.RowIndex).Cells(GC.ImportMessageId).Value) > 0 AndAlso bm.IF_Exists("select dbo.GetAccName(AccNo) from ImportMessages  where OrderTypeId='" & G.Rows(G.CurrentCell.RowIndex).Cells(GC.PurchaseAccNo).Value & "' and Id=" & Val(G.Rows(e.RowIndex).Cells(GC.ImportMessageId).Value) & " and (IsDelivered=1 and DeliveredDate<'" & bm.ToStrDate(DayDate.SelectedDate) & "')") Then
                    bm.ShowMSG("تاريخ الاستلام يسبق تاريخ الحركة")
                    G.Rows(e.RowIndex).Cells(GC.ImportMessageId).Value = 0
                End If
    ...
    Sub ClearControls()
        ...
        If Md.MyProjectType = ProjectType.X23 AndAlso Not DayDate.SelectedDate Is Nothing Then
    ...
    Private Sub txtID_LostFocus(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtID.LostFocus
        ...
        bm.DefineValues()
        Dim dt As New DataTable
        bm.RetrieveAll(New String() {MainId, SubId, SubId2}, New String() {BankCash_G2TypeId.SelectedValue, txtFlag.Text.Trim, txtID.Text}, dt)
        If dt.Rows.Count = 0 Then
            ClearControls()

            dt = bm.ExecuteAdapter("select FromInvoiceNo,ToInvoiceNo from BankCash_G2Types where Flag=" & Flag & " and Id=" & BankCash_G2TypeId.SelectedValue)
            If dt.Rows.Count > 0 Then
    ...
    Private Sub CalcTotal()
        ...
        LopCalc = True
        Try
            Value.Text = Math.Round(0, 13)
            MainValue.Text = Math.Round(0, 13)
            For i As Integer = 0 To G1.Rows.Count - 1
    ...
    Private Sub GridKeyDown(ByVal sender As Object, ByVal e As Forms.KeyEventArgs)
        ...
        Try
            ...
            ElseIf G.CurrentCell.ColumnIndex = G.Columns(GC.SubAccNo).Index Then
                If Md.ShowBankCash_GAccNo_Not_LinkFile Then
                    ...
                    If dt.Rows.Count > 0 AndAlso bm.ShowHelpGrid(dt.Rows(0)("TableName"), G.Rows(G.CurrentCell.RowIndex).Cells(GC.SubAccNo), G.Rows(G.CurrentCell.RowIndex).Cells(GC.SubAccName), e, "select cast(Id as varchar(100)) Id,Name from " & dt.Rows(0)("TableName") & " where AccNo='" & G.Rows(G.CurrentCell.RowIndex).Cells(GC.MainAccNo).Value & "'") Then
                        ...
                        If Val(G.Rows(G.CurrentCell.RowIndex).Cells(GC.LinkFile).Value) = 2 AndAlso Val(G.Rows(G.CurrentCell.RowIndex).Cells(GC.StoreInvoiceNo).Value) = 0 Then
                            'bm.ShowMSG("برجاء إدخال رقم فاتورة المورد")
                        End If
            ...
            ElseIf G.CurrentCell.ColumnIndex = G.Columns(GC.ImportMessageId).Index Then
                If bm.ShowHelpGridMultiColumns("ImportMessages", G.Rows(G.CurrentCell.RowIndex).Cells(GC.ImportMessageId), G.Rows(G.CurrentCell.RowIndex).Cells(GC.ImportMessageId), e, "select cast(Id as varchar(100)) 'رقم الرسالة',dbo.GetShipperName(ShipperId) 'الشاحن',CertificateNo 'رقم الشهادة' from ImportMessages where OrderTypeId='" & G.Rows(G.CurrentCell.RowIndex).Cells(GC.PurchaseAccNo).Value & "'") Then
    ...
    Private Sub SetGCExchange(G As MyGrid)
        If Md.MyProjectType = ProjectType.X23 Then Return
        Try
            If Flag = 2 Then
