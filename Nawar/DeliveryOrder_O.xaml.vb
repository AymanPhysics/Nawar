...
Public Class DeliveryOrder_O
    ...
    Private Sub DocNo_LostFocus(sender As Object, e As RoutedEventArgs) Handles DocNo.LostFocus
        Dim dt2 As DataTable = bm.ExecuteAdapter("GetSalesSerialDetails2", {"Flag", "StoreId", "DocNo", "SalesTypeIdState"}, {Flag, Val(StoreId.Text), Val(DocNo.Text), IIf(Md.MyProjectType = ProjectType.NawarGroup, 0, 1)})
        G.Rows.Clear()
        ...
        Value.Text = bm.ExecuteScalar("select dbo.Bal0(AccNo,Id,getdate(),getdate(),0,0,0) from Customers where ID=" & ToId.Text)
