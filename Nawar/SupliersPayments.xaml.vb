...
Public Class SupliersPayments
    ...
    Public Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles FromDate.SelectedDateChanged, ToDate.SelectedDateChanged
        ...
        G1.MinColumnWidth = 0
        G1.Columns(dt.Columns("Line").Ordinal).Visibility = Windows.Visibility.Hidden
        ...
        G1.Columns(dt.Columns("CheckNo").Ordinal).Header = "رقم الشيك"
    ...
    Private Sub G_SelectionChanged(sender As Object, e As EventArgs)
        ...
        G3.MinColumnWidth = 0
        G3.Columns(dt.Columns("Line").Ordinal).Visibility = Windows.Visibility.Hidden
    ...
    Private Sub GridCalcRow(sender As Object, e As DataGridViewCellEventArgs)
        ...
        ElseIf e.ColumnIndex = G.Columns(GC.InvoiceNo).Index Then
            ...
            Dim dt As DataTable = bm.ExecuteAdapter("select dbo.ToStrDate(Daydate),dbo.fn_ImportInvoiceCostWithoutPayments
(" & G.Rows(e.RowIndex).Cells(GC.StoreId).Value & "," & G.Rows(e.RowIndex).Cells(GC.InvoiceNo).Value & ")/*TotalAfterDiscount*/ from SalesMaster where Flag=19 and StoreId=" & G.Rows(e.RowIndex).Cells(GC.StoreId).Value & " and InvoiceNo=" & G.Rows(e.RowIndex).Cells(GC.InvoiceNo).Value)
    ...
    Sub Calc0()
        ...
        Next
        If Not G1.SelectedItem Is Nothing AndAlso Val(G1.SelectedItem("StoreInvoiceNo")) > 0 Then
            x += G1.SelectedItem("MainValue2")
        End If
        Paid0.Text = x
