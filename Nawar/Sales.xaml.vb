...
Public Class Sales
    ...
    Public Structure FlagState
        ...
        Shared حجز_أصناف_للمبيعات As Integer = 32
    ...
    Function MainFlag() As String
        Select Case Flag
            ...
            Case FlagState.مردودات_مشتريات_خارجية
                Return FlagState.الاستيراد
            Case FlagState.الاستيراد
                Return FlagState.أمر_شراء
            Case Else
                Return 0
        End Select
    End Function
    ...
    Public Sub Sales_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        ...
        If Md.MyProjectType = ProjectType.X5 OrElse Md.MyProjectType = ProjectType.X6 Then
            btnNew.Content &= " (F8)"
        ...
        If Not Md.MyProjectType = ProjectType.X22 Then
        ...
        If Not (Md.MyProjectType = ProjectType.NawarGroup AndAlso Flag = FlagState.مشتريات) Then
        ...
        End If
        If Md.MyProjectType = ProjectType.NawarGroup AndAlso Flag = FlagState.المبيعات Then
            If btnPrint.Content <> "طباعة كشف وإذن صرف" Then
        ...
        End If
        If Md.MyProjectType = ProjectType.X23 Then
            CurrencyId.IsEnabled = True
        End If
        If Md.MyProjectType <> ProjectType.X26 OrElse (Flag <> FlagState.المبيعات AndAlso Flag <> FlagState.مبيعات_لمستثمر) Then
            btnInstallment.Visibility = Windows.Visibility.Hidden
        End If

        If Md.MyProjectType <> ProjectType.X9 OrElse Flag <> FlagState.تحويل_إلى_مخزن Then
            btnReplacement.Visibility = Windows.Visibility.Hidden
        End If
    ...
    Private Sub LoadWFH()
        ...
        G.Columns.Add(GC.SalesInvoiceNo, IIf(TestImportOnly, "أمر الشراء", "رقم الفاتورة"))
        ...
        If Flag <> FlagState.تحويل_إلى_مخزن OrElse Md.MyProjectType <> ProjectType.X9 Then
        ...
        If Md.MyProjectType = ProjectType.NawarGroup OrElse Md.MyProjectType = ProjectType.X28 Then
        ...
        Select Case Flag
            Case FlagState.مردودات_الاستيراد, FlagState.مردودات_التصدير, FlagState.مردودات_المبيعات, FlagState.مردودات_المستهلكات, FlagState.مردودات_مبيعات_التوصيل, FlagState.مردودات_مبيعات_الجملة, FlagState.مردودات_مبيعات_الصالة, FlagState.مردودات_مبيعات_نصف_الجملة, FlagState.مردودات_مشتريات, FlagState.مردودات_مشتريات_خارجية, FlagState.الاستيراد
            Case Else
                G.Columns(GC.SalesInvoiceNo).Visible = False
        End Select

        If Md.MyProjectType = ProjectType.NawarGroup AndAlso Flag = FlagState.تحويل_إلى_مخزن Then
        ...
        If Md.MyProjectType = ProjectType.NawarGroup Then
            If (TestPurchaseAndReturn() AndAlso Not Md.UserSeePurchasesPrice) _
               ...
               OrElse Flag = FlagState.تحويل_إلى_مخزن _
               OrElse Flag = FlagState.إضافة _
               OrElse Flag = FlagState.صرف Then
        ...
        If Not (Md.MyProjectType = ProjectType.X22 OrElse Md.MyProjectType = ProjectType.X28 OrElse Md.MyProjectType = ProjectType.X23) Then
        ...
        If Not (Md.MyProjectType = ProjectType.X27 AndAlso TestImportOnly()) Then
    ...
    Function ReadOnlyState() As Boolean
        If Md.MyProjectType = ProjectType.NawarGroup AndAlso (Flag = FlagState.تسوية_صرف OrElse Flag = FlagState.صرف) Then
            Return True
        ElseIf Md.Manager OrElse Md.EditPrices Then
            Return False
        ElseIf Md.MyProjectType = ProjectType.X14 OrElse Md.MyProjectType = ProjectType.X1 Then
            Return False
    ...
    Sub LoadAllItems()
        Try
            ...
            If (Md.Receptionist AndAlso Md.MyProjectType = ProjectType.X15) OrElse (Not Md.Manager AndAlso Md.Nurse AndAlso Md.MyProjectType = ProjectType.X9) Then
    ...
    Function ItemWhere() As String
        ...
        If Md.MyProjectType = ProjectType.NawarGroup AndAlso TestSalesAndReturn() Then
            st = " and ItemType in(2,3) "
        End If

        If TestConsumablesAndReturn() AndAlso Not Md.MyProjectType = ProjectType.X9 Then
            st &= " and (Flag=1 or IsService=1) "
        End If

        If Not TestSalesAndReturn() AndAlso Not TestConsumablesAndReturn() AndAlso Not Md.MyProjectType = ProjectType.X9 Then
            st &= " and IsService=0 "
        End If
        Return st
    End Function
    Private Sub LoadItems(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Try
            ...
            Dim dt As DataTable = bm.ExecuteAdapter("Select * From Items_View  where IsStopped=0 " & ItemWhere() & " and GroupId=" & WTypes.Tag.ToString & " and TypeId=" & xx.Tag.ToString & " order by " & IIf(Md.MyProjectType = ProjectType.X22, "Id", "Name"))
            For i As Integer = 0 To dt.Rows.Count - 1
                Dim x As New Button
                bm.SetStyle(x, 370)
                'bm.SetImage(x, CType(dt.Rows(i)("Image"), Byte()))
...
Br:
            ...
            If (Md.MyProjectType = ProjectType.NawarGroup OrElse Md.MyProjectType = ProjectType.X0) AndAlso (Flag = FlagState.إضافة OrElse Flag = FlagState.مشتريات OrElse Flag = FlagState.الاستيراد OrElse Flag = FlagState.تحويل_إلى_مخزن OrElse Flag = FlagState.المبيعات) Then
                If i = 0 Then
                    Dim x As Integer = 1
                    Select Case Flag
                        Case FlagState.تحويل_إلى_مخزن
    ...
    Sub CalcRow(ByVal i As Integer, Optional ForceValid As Boolean = False)
        Try
            ...
            If Not lop AndAlso (Flag = FlagState.المبيعات OrElse Flag = FlagState.مردودات_مشتريات OrElse Flag = FlagState.مردودات_مشتريات_خارجية OrElse Flag = FlagState.تحويل_إلى_مخزن OrElse Flag = FlagState.عرض_أسعار OrElse Flag = FlagState.أمر_توريد OrElse Flag = FlagState.حجز_أصناف_للمبيعات OrElse Flag = FlagState.صرف OrElse Flag = FlagState.تسوية_صرف) AndAlso Val(G.Rows(i).Cells(GC.Qty).Value) > Val(ItemRem.Text) Then
                If G.CurrentCell.ColumnIndex = G.Columns(GC.Qty).Index OrElse ForceValid Then
                    bm.ShowMSG("رصيد الصنف لا يسمح")
                End If
                If Md.MyProjectType = ProjectType.NawarGroup OrElse Md.MyProjectType = ProjectType.X26 Then
    ...
    Sub CalcResRem(i As Integer)
        ItemBal.Text = G.Rows(i).Cells(GC.CurrentBal).Value
        ItemRes.Text = bm.ExecuteScalar("select dbo.GetItemRes(" & Val(StoreId.Text) & "," & G.Rows(i).Cells(GC.Id).Value & ",'" & bm.ToStrDate(DayDate.SelectedDate) & "')")
        ItemRem.Text = Val(ItemBal.Text) - Val(ItemRes.Text)
        Limit.Text = bm.ExecuteScalar("select Limit from Items where Id=" & G.Rows(i).Cells(GC.Id).Value)
    End Sub
    ...
    Private Sub RdoCash_Checked(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles RdoCash.Checked, RdoVisa.Checked, RdoCashVisa.Checked, RdoFuture.Checked, RdoCashFuture.Checked, RdoEmployees.Checked
        If lblGroupBoxPaymentType Is Nothing OrElse sender Is Nothing Then Return
        Try
        ...
        Try
            If (RdoCash.IsChecked OrElse RdoCashFuture.IsChecked OrElse RdoCashVisa.IsChecked) AndAlso (TestPurchaseAndReturn() OrElse TestSalesAndReturn() OrElse TestExportAndReturn()) Then
                If Not Md.MyProjectType = ProjectType.X21 Then
    ...
    Private Sub GridCalcRow(ByVal sender As Object, ByVal e As Forms.DataGridViewCellEventArgs)
        ...
        Try
            ...
            ElseIf G.Columns(e.ColumnIndex).Name = GC.ItemSerialNo AndAlso Not G.Rows(e.RowIndex).Cells(GC.ItemSerialNo).Value Is Nothing AndAlso Not G.Rows(e.RowIndex).Cells(GC.ItemSerialNo).Value = "" Then
                ...
                If dt.Rows.Count > 0 Then
                    If Md.MyProjectType = ProjectType.X26 AndAlso TestPurchaseOnly() Then
            ...
            ElseIf G.Columns(e.ColumnIndex).Name = GC.Barcode AndAlso Not G.Rows(e.RowIndex).Cells(GC.Barcode).Value Is Nothing AndAlso Not G.Rows(e.RowIndex).Cells(GC.Barcode).Value = "" Then
                ...
                If (Md.MyProjectType = ProjectType.X15) AndAlso Not G.Rows(e.RowIndex).Cells(GC.Barcode).Value = Nothing Then
    ...
    Private Sub OrderTypeId_LostFocus(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles OrderTypeId.LostFocus
        ...
        If TestImportAndReturn() AndAlso Not TestOuterPurchaseAndReturn() Then bm.LostFocus(OrderTypeId, AccNo, "select AccNo1 from OrderTypes where Id=" & OrderTypeId.Text.Trim(), True)
    End Sub
    ...
    Private Sub ToId_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles ToId.KeyUp
        ...
        ElseIf ((TestPurchaseAndReturn() OrElse TestImportAndReturn()) AndAlso Not ReservToId.IsChecked) OrElse ((TestSalesAndReturn() OrElse TestExportAndReturn()) AndAlso ReservToId.IsChecked) Then
            tbl = "Suppliers"
            Title = "الموردين"
            If bm.ShowHelp(Title, ToId, ToName, e, "select cast(Id as varchar(100)) Id,Name from " & tbl, IIf(Md.MyProjectType = ProjectType.X21 OrElse Md.MyProjectType = ProjectType.X9, tbl, "")) Then
                ToId_LostFocus(sender, Nothing)
            End If
        ...
        ElseIf TestConsumablesAndReturn() Then
            If bm.ShowHelpCases(ToId, ToName, e, IIf(Flag = FlagState.المستهلكات OrElse Flag = FlagState.مردودات_المستهلكات, False, True), IIf(Md.MyProjectType = ProjectType.X12, False, True)) Then
                ToId_LostFocus(sender, Nothing)
            End If
        End If
    End Sub
    ...
    Function TestSalesAndReturn() As Boolean
        Return (Flag = FlagState.مبيعات_الصالة OrElse Flag = FlagState.مردودات_مبيعات_الصالة _
                ...
                OrElse Flag = FlagState.عرض_أسعار OrElse Flag = FlagState.أمر_توريد OrElse Flag = FlagState.حجز_أصناف_للمبيعات)
    End Function
    ...
    Function TestPurchaseAndReturn() As Boolean
        Return (Flag = FlagState.مشتريات OrElse Flag = FlagState.مردودات_مشتريات OrElse Flag = FlagState.أمر_شراء OrElse Flag = FlagState.عرض_أسعار_مورد)
    End Function
    ...
    Function TestImportAndReturn() As Boolean
        Return (Flag = FlagState.الاستيراد OrElse Flag = FlagState.مردودات_الاستيراد OrElse Flag = FlagState.مشتريات_خارجية OrElse Flag = FlagState.مردودات_مشتريات_خارجية)
    End Function
    ...
    Private Sub ToId_LostFocus(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles ToId.LostFocus
        ...
        If Md.MyProjectType = ProjectType.NawarGroup And TestSalesAndReturn() And Not lop Then
        ...
        If Md.MyProjectType = ProjectType.X28 AndAlso (TestSalesAndReturn() OrElse TestPurchaseAndReturn()) Then
            CalcBalSub()
        End If
        ...
        If ((TestSalesAndReturn() OrElse TestExportAndReturn()) AndAlso Not ReservToId.IsChecked) OrElse ((TestPurchaseAndReturn()) AndAlso ReservToId.IsChecked) Then
            ...
            End If
        End If
        If TestImportAndReturn() OrElse TestExportAndReturn() OrElse (Md.MyProjectType = ProjectType.X9 AndAlso TestPurchaseAndReturn()) Then
            RdoCash.IsChecked = True
            RdoFuture.IsChecked = True
        End If
    ...
    Private Sub CashierId_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles CashierId.KeyUp
        bm.ShowHelp("Employees", CashierId, CashierName, e, "select cast(Id as varchar(100)) Id," & Resources.Item("CboName") & " Name from Employees where Doctor=0 and Stopped=0" & IIf(TestConsumablesAndReturn() AndAlso Md.MyProjectType = ProjectType.X12, " and Nurse=1", " and Cashier=1"))
    End Sub
    ...
    Private Sub CashierId_LostFocus(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles CashierId.LostFocus
        bm.LostFocus(CashierId, CashierName, "select " & Resources.Item("CboName") & " Name from Employees where Doctor=0 and Id=" & CashierId.Text.Trim() & " and Stopped=0 " & IIf(TestConsumablesAndReturn() AndAlso Md.MyProjectType = ProjectType.X12, " and Nurse=1", " and Cashier=1"))
    End Sub
    ...
    Sub FillControls()
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            ...
            If TestSalesOnly() And Not Flag = FlagState.حجز_أصناف_للمبيعات Then
                G.Rows(i).Cells(GC.CurrentBal).Value += G.Rows(i).Cells(GC.Qty).Value
            ElseIf TestPurchaseOnly() Then
                G.Rows(i).Cells(GC.CurrentBal).Value -= G.Rows(i).Cells(GC.Qty).Value
            Else
        ...
        If IsPosted.IsChecked OrElse Val(CaseInvoiceNo.Text) > 0 OrElse Val(NewInvoiceNo.Text) > 0 Then
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles btnPrint.Click, btnPrint2.Click, btnPrint3.Click, btnPrint4.Click, btnPrint5.Click, btnPrintImage.Click, btnReceiveSave.Click, btnPrintSafe.Click
        If Md.Manager OrElse Md.MyProjectType = ProjectType.X24 Then DontClear = True
        If Md.MyProjectType = ProjectType.X28 Then DontClear = False
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If Md.MyProjectType = ProjectType.NawarGroup AndAlso DocNo.Text.Trim = "" And Flag <> FlagState.عرض_أسعار And Flag <> FlagState.أمر_شراء And Flag <> FlagState.عرض_أسعار_مورد And Flag <> FlagState.أمر_توريد And Flag <> FlagState.حجز_أصناف_للمبيعات Then
        ...
        If btnSave.IsEnabled Then
            ...
            If Md.MyProjectType = ProjectType.X1 AndAlso TestSalesAndReturn() Then
        ...
        If Md.MyProjectType = ProjectType.X28 AndAlso TestSalesAndReturn() Then
            SaveCustomerData()
        End If
...
Print:
        ...
        If sender Is btnPrint OrElse sender Is btnPrint2 OrElse sender Is btnPrint3 OrElse sender Is btnPrint4 OrElse sender Is btnPrint5 OrElse sender Is btnPrintImage OrElse sender Is btnPrintSafe OrElse (sender Is btnCloseTable And btnPrint.IsEnabled) Then
            PrintPone(sender, 0)
            If Md.MyProjectType = ProjectType.NawarGroup AndAlso sender Is btnPrint AndAlso Flag = FlagState.المبيعات Then
    ...
    Sub ClearControls()
        Try
            ...
            If Md.MyProjectType = ProjectType.X22 AndAlso TestSalesAndReturn() Then
            ...
            If Temp2.Visibility = Visibility.Visible Then
            ...
            If SalesTypeId.Items.Count > 0 Then
                ...
                If Md.MyProjectType = ProjectType.X22 AndAlso TestSalesOnly() Then
            ...
            If Md.MyProjectType <> ProjectType.X12 AndAlso Md.MyProjectType <> ProjectType.X2 AndAlso Md.MyProjectType <> ProjectType.X3 AndAlso Md.MyProjectType <> ProjectType.NawarGroup Then CashierId.Text = Md.UserName
            ...
            If TestPurchaseAndReturn() Then
                If Md.MyProjectType = ProjectType.X24 AndAlso TestOuterPurchaseAndReturn() Then
                Else
            ...
            If Md.MyProjectType = ProjectType.X15 AndAlso TestSalesOnly() Then
                Temp.Visibility = Windows.Visibility.Visible
                Temp.Content = "فاتورة حجز"
            ElseIf Md.MyProjectType = ProjectType.X15 AndAlso TestPurchaseOnly() Then
                Temp.Visibility = Windows.Visibility.Visible
            ...
            If Md.MyProjectType = ProjectType.X3 Then
                Temp.Visibility = Windows.Visibility.Hidden
            End If

            'If TestImportOnly() Then
            '    Temp2.Visibility = Windows.Visibility.Visible
            '    Temp2.IsChecked = False
            'End If
            ...
            If Not Md.Manager Then
                ...
                If Not Md.MyProjectType = ProjectType.NawarGroup Then CashierId.Text = Md.UserName
                CashierId_LostFocus(Nothing, Nothing)
            Else
        ...
        If TestSalesAndReturn() AndAlso Md.MyProjectType = ProjectType.NawarGroup Then
    ...
    Private Sub InvoiceNo_KeyUp(sender As Object, e As KeyEventArgs) Handles InvoiceNo.KeyUp
        ...
        ElseIf TestSalesAndReturn() OrElse TestExportAndReturn() Then
            ...
            If TypeOf (Parent) Is Page Then
                MyTitle = CType(Parent, Page).Title
            ElseIf TypeOf (Parent) Is MyWindow Then
                MyTitle = CType(Parent, MyWindow).Title
            End If
            If bm.ShowHelpMultiColumns(MyTitle, InvoiceNo, InvoiceNo, e, "select cast(InvoiceNo as nvarchar(100))'رقم الفاتورة',cast(ToId as nvarchar(100))'كود العميل',dbo.GetCustomerName(ToId)'اسم العميل',dbo.ToStrDate(DayDate)'التاريخ',DocNo 'رقم المستند' from SalesMaster where StoreId=" & Val(StoreId.Text) & " and Flag=" & Flag.ToString) Then
                'InvoiceNo.Text = bm.SelectedRow(0)
                InvoiceNo_Leave(Nothing, Nothing)
            End If
        End If
    ...
    Private Sub PrintPone(ByVal sender As System.Object, ByVal NewItemsOnly As Integer)
        ...
        If TestSalesAndReturn() OrElse TestPurchaseAndReturn() Then
            Select Case Md.MyProjectType
                Case ProjectType.NawarGroup, ProjectType.X28
                    MyBal = Val(bm.ExecuteScalar("select dbo.Bal0(AccNo,Id,'" & bm.ToStrDate(DayDate.SelectedDate) & "','" & bm.ToStrDate(DayDate.SelectedDate) & "',0,0,0) from " & tbl & " where Id=" & ToId.Text))
            End Select
        End If
        ...
        If NewItemsOnly = 0 Then
            ...
            ElseIf TypeOf (Parent) Is MyWindow Then
                MyTitle = CType(Parent, MyWindow).Title
            End If
            rpt.paravalue = New String() {DayDate.SelectedDate, DayDate.SelectedDate, Val(Shift.SelectedValue), Flag, StoreId.Text, InvoiceNo.Text, InvoiceNo.Text, InvoiceNo.Text, NewItemsOnly, 0, 0, 0, MyTitle, Val(Remaining.Text), Val(Payed.Text), MyBal}
            ...
            If sender Is btnPrint3 AndAlso Md.MyProjectType = ProjectType.X15 Then rpt.Rpt = "SalesPone33.rpt"
            If sender Is btnPrint3 AndAlso Md.MyProjectType = ProjectType.X0 Then rpt.Rpt = "SalesPone3_H.rpt"
            If sender Is btnPrint3 AndAlso Md.MyProjectType = ProjectType.X27 Then rpt.Rpt = "SalesPone3_O.rpt"
            ...
            If sender Is btnPrint2 Then
                ...
                rpt.Rpt = IIf(Md.MyProjectType = ProjectType.X28, "SalesPone2_AZ.rpt", rpt.Rpt)
            ...
            If sender Is btnPrint5 Then
                ...
                ElseIf Md.MyProjectType = ProjectType.X28 Then
                ...
                If Md.MyProjectType = ProjectType.NawarGroup Then
                    ...
                    Else
                        rpt.Show()
                    End If
                ElseIf Md.MyProjectType = ProjectType.X27 Then
            ...
            If Md.MyProjectType = ProjectType.NawarGroup AndAlso sender Is btnPrint Then
                rpt.ShowDialog()
            Else
                rpt.Show()
            End If
    ...
    Sub CalcTotalEnd() Handles Per1.TextChanged, Per2.TextChanged, Per3.TextChanged, Per4.TextChanged, Val1.LostFocus, Val2.LostFocus, Val3.LostFocus, Val4.LostFocus, AccType1.SelectionChanged, AccType2.SelectionChanged, AccType3.SelectionChanged, AccType4.SelectionChanged, Shipping.LostFocus, Freight.LostFocus, CustomClearance.LostFocus
        ...
        If Md.MyProjectType.X24 And TestSalesAndReturn() Then
    ...
    Private Sub GridKeyDown(ByVal sender As Object, ByVal e As Forms.KeyEventArgs)
        e.Handled = True
        If Receive OrElse G.CurrentCell Is Nothing Then Return
        Try
            ...
            If G.CurrentCell.ColumnIndex = G.Columns(GC.SalesInvoiceNo).Index Then
                ...
                If bm.ShowHelpGridMultiColumns("Invoices", G.CurrentRow.Cells(GC.SalesInvoiceNo), G.CurrentRow.Cells(GC.SalesInvoiceNo), e, "select distinct cast(M.InvoiceNo as nvarchar(100)) 'الفاتورة',dbo.ToStrDate(M.DayDate) 'التاريخ',cast(D.ItemId as nvarchar(100)) 'كود الصنف',D.ItemName 'اسم الصنف',cast(D.Qty as nvarchar(100)) 'الكمية',cast(D.Price -D.ItemDiscount as nvarchar(100)) 'السعر بعد الخصم',cast(D.Value as nvarchar(100)) 'القيمة' from SalesMaster M left join SalesDetails D on(M.Flag=D.Flag and M.StoreId=D.StoreId and M.InvoiceNo=D.InvoiceNo) where M.Temp=0 and M.StoreId=" & StoreId.Text & " and M.Flag=" & MainFlag() & " and (M.ToId=" & Val(ToId.Text) & " or " & Val(ToId.Text) & "=0) and (M.OrderTypeId=0 or M.OrderTypeId=" & Val(OrderTypeId.Text) & " or " & Val(OrderTypeId.Text) & "=0) and (D.ItemId=" & Val(G.Rows(G.CurrentCell.RowIndex).Cells(GC.Id).Value) & " or " & Val(G.Rows(G.CurrentCell.RowIndex).Cells(GC.Id).Value) & "=0)") Then
                    'GridCalcRow(sender, New Forms.DataGridViewCellEventArgs(G.Columns(GC.SalesInvoiceNo).Index, G.CurrentCell.RowIndex))
    ...
    Private Sub HideAcc()
        ...
        If Md.MyProjectType = ProjectType.X22 Then
    ...
    Private Sub LoadVisibility()
        ...
        btnDeliveryOrder.Visibility = Windows.Visibility.Hidden
        ...
        DocNo2.Visibility = Visibility.Hidden
        lblDocNo2.Visibility = Visibility.Hidden
        ...
        Select Case Flag
            Case FlagState.المبيعات, FlagState.الاستيراد, FlagState.مردودات_الاستيراد, FlagState.عرض_أسعار, FlagState.أمر_توريد, FlagState.أمر_شراء, FlagState.حجز_أصناف_للمبيعات
        ...
        ElseIf TestImportAndReturn() Then
            ...
            lblDocNo2.Visibility = Visibility.Visible
            DocNo2.Visibility = Visibility.Visible
            lblDocNo2.Content = "رقم إذن صرف المورد"
            ...
            If Md.MyProjectType = ProjectType.NawarGroup OrElse Md.MyProjectType = ProjectType.X0 Then
                lblDeliveryDate.Visibility = Windows.Visibility.Visible
                DeliveryDate.Visibility = Windows.Visibility.Visible
            ...
            btnPrint3.Content = "إذن إضافة"
            'HideAcc()
            If Flag = FlagState.مردودات_مشتريات_خارجية Then
                Temp2.Visibility = Visibility.Visible
            End If

        ElseIf TestSalesAndReturn() OrElse TestExportAndReturn() Then

            If Md.MyProjectType = ProjectType.NawarGroup OrElse Md.MyProjectType = ProjectType.X0 OrElse Md.MyProjectType = ProjectType.X24 Then
                lblDeliveryDate.Visibility = Windows.Visibility.Visible
                DeliveryDate.Visibility = Windows.Visibility.Visible
                If Md.MyProjectType = ProjectType.NawarGroup Then
                    lblDeliveryDate.Content = "تاريخ التحميل"
                ElseIf Md.MyProjectType = ProjectType.X24 Then
            ...
            If Flag <> FlagState.عرض_أسعار AndAlso Flag <> FlagState.أمر_توريد AndAlso Flag <> FlagState.حجز_أصناف_للمبيعات Then
                lblCashier.Visibility = Visibility.Visible
                CashierId.Visibility = Visibility.Visible
                CashierName.Visibility = Visibility.Visible
            ...
            If Not (Md.MyProjectType = ProjectType.X5 OrElse Md.MyProjectType = ProjectType.X6 OrElse Md.MyProjectType = ProjectType.X21 OrElse Md.MyProjectType = ProjectType.X23 OrElse Md.MyProjectType = ProjectType.NawarGroup) Then
            ...
            If TestSalesOnly() Then
                btnDeliveryOrder.Visibility = Windows.Visibility.Visible
            End If
            If Md.MyProjectType = ProjectType.X27 Then
        ...
        If Not Flag = FlagState.مشتريات OrElse Not Md.MyProjectType = ProjectType.X12 Then
        ...
        If Not Md.Manager AndAlso Not Md.MyProjectType = ProjectType.X12 Then
        ...
        If Not TestSalesAndReturn() OrElse Flag = FlagState.عرض_أسعار OrElse Flag = FlagState.أمر_توريد OrElse Flag = FlagState.أمر_شراء OrElse Flag = FlagState.عرض_أسعار_مورد OrElse Flag = FlagState.حجز_أصناف_للمبيعات Then
        ...
        If Md.MyProjectType = ProjectType.X12 OrElse Md.MyProjectType = ProjectType.X2 OrElse Md.MyProjectType = ProjectType.X3 Then
            Payed.Visibility = Windows.Visibility.Hidden
            lblPayed.Visibility = Windows.Visibility.Hidden
            Remaining.Visibility = Windows.Visibility.Hidden
            lblRemaining.Visibility = Windows.Visibility.Hidden
        ...
        If Flag = FlagState.عرض_أسعار OrElse Flag = FlagState.أمر_توريد OrElse Flag = FlagState.أمر_شراء OrElse Flag = FlagState.عرض_أسعار_مورد OrElse Flag = FlagState.حجز_أصناف_للمبيعات Then
        ...
        If (Md.MyProjectType = ProjectType.NawarGroup OrElse Md.MyProjectType = ProjectType.X0) AndAlso Flag <> FlagState.مشتريات AndAlso Flag <> FlagState.مردودات_مشتريات AndAlso Flag <> FlagState.مشتريات_خارجية AndAlso Flag <> FlagState.مردودات_مشتريات_خارجية AndAlso Flag <> FlagState.الاستيراد AndAlso Flag <> FlagState.مردودات_الاستيراد Then
            DocNo.Visibility = Visibility.Hidden
            lblDocNo.Visibility = Visibility.Hidden
        End If
    ...
    Public Sub G_SelectionChanged(sender As Object, e As EventArgs)
        If lop And Not LopRes Then Return

        If G.CurrentRow.Cells(GC.Id).Value Is Nothing OrElse IsClearing Then
            ItemBal.Clear()
            ItemRes.Clear()
            ItemRem.Clear()
    ...
    Public Sub calcAllBal()
        For i As Integer = 0 To G.Rows.Count - 1
            GetItemNameAndBal(i, G.Rows(i).Cells(GC.Id).Value, G.Rows(i).Cells(GC.ItemSerialNo).Value)
        Next
    End Sub

    Private Sub DocNo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles DocNo.KeyUp
        If Not (Md.MyProjectType = ProjectType.NawarGroup AndAlso TestPurchaseOnly()) Then Return
    ...
    Private Sub btnGenerateSalesInvoiceFromQuotation_Click(sender As Object, e As RoutedEventArgs) Handles btnGenerateSalesInvoiceFromQuotation.Click
        If Flag = FlagState.عرض_أسعار_مورد Then
            If bm.ShowDeleteMSG("هل أنت متأكد من إنشاء فاتورة مشتريات؟") Then
                ...
                btnSave_Click(sender, e)
                DontClear = False
                If AllowSave Then bm.ShowMSG("تم إنشاء فاتورة مشتريات برقم " & bm.ExecuteScalar("GeneratePurchaseInvoiceFromQuotation", {"StoreId", "InvoiceNo"}, {Val(StoreId.Text), Val(InvoiceNo.Text)}))
            End If
        ElseIf Flag = FlagState.أمر_توريد Then
            If bm.ShowDeleteMSG("هل أنت متأكد من إنشاء فاتورة مبيعات؟") Then
                ...
                btnSave_Click(sender, e)
                DontClear = False
                If AllowSave Then bm.ShowMSG("تم إنشاء فاتورة مبيعات برقم " & bm.ExecuteScalar("GenerateSalesInvoiceFromQuotation2", {"StoreId", "InvoiceNo"}, {Val(StoreId.Text), Val(InvoiceNo.Text)}))
            End If
        ElseIf Flag = FlagState.حجز_أصناف_للمبيعات Then
            If bm.ShowDeleteMSG("هل أنت متأكد من إنشاء فاتورة مبيعات؟") Then
                DontClear = True
                btnSave_Click(sender, e)
                DontClear = False
                ...
                Dim s As String = ""
                If AllowSave Then
                    s = bm.ExecuteScalar("GenerateSalesInvoiceFromReservation", {"StoreId", "InvoiceNo"}, {Val(StoreId.Text), Val(InvoiceNo.Text)})
                    'bm.ShowMSG("تم إنشاء فاتورة مبيعات برقم " & s)
                    Dim c As New Sales With {.Flag = Sales.FlagState.المبيعات}
                    Dim w As New MyWindow With {.Content = c, .Title = "المبيعات"}
                    Dim id As Integer = 549
                    ...
                    c.StoreId.Text = Val(StoreId.Text)
                    ...
                    c.InvoiceNo.Text = ""
                    bm.ExecuteNonQuery("delete from " & TableName & " where " & SubId & "='" & s & "' and " & MainId & " ='" & StoreId.Text & "'" & " and Flag=" & 13)

                    bm.ExecuteNonQuery("delete from " & TableDetailsName & " where " & SubId & "='" & s & "' and " & MainId & " ='" & StoreId.Text & "'" & " and Flag=" & 13)
                    NewInvoiceNo.Text = s
        ...
        Else
            If bm.ShowDeleteMSG("هل أنت متأكد من إنشاء فاتورة مبيعات؟") Then
                ...
                btnSave_Click(sender, e)
                DontClear = False
                If AllowSave Then bm.ShowMSG("تم إنشاء فاتورة مبيعات برقم " & bm.ExecuteScalar("GenerateSalesInvoiceFromQuotation", {"StoreId", "InvoiceNo"}, {Val(StoreId.Text), Val(InvoiceNo.Text)}))
            End If
        End If
    ...
    Private Sub NewInvoiceNo_TextChanged(sender As Object, e As TextChangedEventArgs) Handles NewInvoiceNo.TextChanged
        btnGenerateSalesInvoiceFromQuotation.IsEnabled = Val(NewInvoiceNo.Text) = 0
        btnSave.IsEnabled = Val(NewInvoiceNo.Text) = 0
        btnDelete.IsEnabled = Val(NewInvoiceNo.Text) = 0
    End Sub
    ...
    Private Sub BtnMonthlyPayment_Click(sender As Object, e As RoutedEventArgs) Handles btnMonthlyPayment.Click
        ...
        rpt.paravalue = New String() {Val(ToId.Text), rpt.Header}
    ...
    Private Sub btnDeliveryOrder_Click(sender As Object, e As RoutedEventArgs) Handles btnDeliveryOrder.Click
        Dim frm As New MyWindow With {.Title = "DeliveryOrder", .WindowState = WindowState.Maximized}
        bm.SetMySecurityType(frm, 558)
        frm.Content = New DeliveryOrder With {.MyDocNo = DocNo.Text}
        frm.Show()
    End Sub
